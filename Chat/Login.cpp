...
void Login::requstGateCallback(pc_request_t *req, int status, json_t *resp)
{
    if(status == -1) {
        ...
        //!! 注意，gate的端口是3014，而登录返回回来的端口是3050
        //!! 返回的是connector的端口
...
{	
    ...
    pc_client_join(client);//!!请求的时候新开启一个线程，等待该线程结束了再退出login
...
//!! 层有一个虚函数，文本框变化时会调用，但是实现中要区分时哪个文本框变化了！
//!! 把变化的值填入变量
